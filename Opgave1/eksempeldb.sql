...
postnr char(4) 
foreign key references postnummer(postnr) not null
)
...
create table ansati
(
cpr char(10) constraint cprforeign 
foreign key references person(cpr),
firmanr int constraint firmaforeign foreign key references firma(firmanr) primary key(cpr,firmanr)
)
